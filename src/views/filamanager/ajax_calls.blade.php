        ...
        case 'get_file': // preview or edit
            ...
            if ($sub_action == 'preview') {
                if ($preview_mode == 'text') {
                    ...
                    $ret .= '<?prettify lang='.$info['extension'].' linenums=true?>
<pre class="prettyprint"><code class="language-'.$info['extension'].'">'.$data.'</code></pre>';
} elseif ($preview_mode == 'google') {
if ($ftp) {
$url_file = $selected_file;
} else {
...
$ret = "<iframe src=\"https://docs.google.com/viewer?url=" . $url_file . "&embedded=true\"
...
<script>
    setTimeout(function() {
        ClassicEditor.create(document.querySelector("#textfile_edit_area")).catch(function(error) {
            console.error(error);
        });
    }, 500);
</script>';
