...
if (!function_exists('trans')) {
     ...
     if (
          ...
          || file_exists('vendor/filemanager/lang/' . basename($_SESSION['RF']['language']) . '.php') === false
          || !is_readable('vendor/filemanager/lang/' . basename($_SESSION['RF']['language']) . '.php')
     ) {
          ...
          if ($lang != $config['default_language']) {
               ...
               $languages = include 'vendor/filemanager/lang/languages.php';
          }

          // add lang file to session for easy include
          $_SESSION['RF']['language'] = $lang;
     } else {
          if (file_exists('vendor/filemanager/lang/languages.php')) {
               $languages = include 'vendor/filemanager/lang/languages.php';
          } else {
               $languages = include '../vendor/crudbooster/filemanager/includes/lang/languages.php';
          }
     ...
     if (file_exists('vendor/filemanager/lang/' . $lang . '.php')) {
          $lang_vars = include 'vendor/filemanager/lang/' . $lang . '.php';
     } else {
          $lang_vars = include '../vendor/crudbooster/filemanager/includes/lang/' . $lang . '.php';
     }
...
function ftp_con($config)
{
     if (isset($config['ftp_host']) && $config['ftp_host']) {
          // *** Include the class
          include(public_path() . '/vendor/voila_crocodicstudio/crudbooster/filemanager/includes/include/FtpClient.php');
          include(public_path() . '/vendor/voila_crocodicstudio/crudbooster/filemanager/includes/include/FtpException.php');
          include(public_path() . '/vendor/voila_crocodicstudio/crudbooster/filemanager/includes/include/FtpWrapper.php');
...
{
     $config = include public_path() . '/vendor/voila_crocodicstudio/crudbooster/filemanager/includes/config/config.php';
     $total_size = 0;
     $files = scandir($path);
...
{
     $config = include include public_path() . '/vendor/voila_crocodicstudio/crudbooster/filemanager/includes/config/config.php';
     $total_count = 0;
     $files = scandir($path);
     $cleanPath = rtrim($path, '/') . '/';
